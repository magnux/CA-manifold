...
model_manager = ModelManager('regae_cross', networks_dict, config)  #, to_avg=['encoder', 'decoder'])
...
# # Distributions
# zdist_mu, zdist_cov = load_multigauss_params(config['training']['out_dir'] + '/regae_cross', lat_size, device=device)
# zdist_cov_inv = None
...
for _ in range(model_manager.epoch, n_epochs):
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                if config['training']['stream_every'] > 0 and (model_manager.it % config['training']['stream_every']) == 0:
                    ...
                    stream_images(images_dec, config_name + '/regae_cross', config['training']['out_dir'] + '/regae_cross')
    ...
    # save_multigauss_params(zdist_mu, zdist_cov, config['training']['out_dir'] + '/regae_cross')
