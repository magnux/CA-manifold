...
def load_dataset(root):
    root = os.path.join(root, 'MNIST/raw')
...
class MovingMnistDataset(data.Dataset):
    def __init__(self, root, seq_len=20, nums_per_image=2, fake_dataset_size=1024, speed=5, speed_type='random', reflect_end=False):
        self.epoch_length = fake_dataset_size
        self.seq_len = seq_len
        self.mnist = load_dataset(root)
        ...
        self.reflect_end = reflect_end

    def __getitem__(self, index):
        # randomly generate direc/speed/position, calculate velocity vector
        direcs = np.pi * (np.random.rand(self.nums_per_image) * 2 - 1)
        if self.speed_type == 'fixed':
            speeds = np.array([self.speed] * self.nums_per_image)
        elif self.speed_type == 'random':
            speeds = np.random.randint(self.speed, size=self.nums_per_image) + 2
        ...
        data = np.zeros((self.seq_len * 2 - 1 if self.reflect_end else self.seq_len, 64, 64))
        ...
        if self.reflect_end:
            for frame_idx in range(self.seq_len, self.seq_len * 2 - 1):
                data[frame_idx, ...] = data[self.seq_len * 2 - 2 - frame_idx, ...]
...
if __name__ == '__main__':
    ds = MovingMnistDataset('data/mnist', seq_len=4, speed=1, speed_type='fixed', reflect_end=True)
    # print(ds[0])
    import h5py
