...
class InjectedEncoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, ds_size, channels, n_filter, z_out=False, z_dim=0, auto_reg=False, **kwargs):
        ...
        self.conv_img = MixConv(self.lat_size, self.in_chan, self.n_filter * (2 ** self.n_layers), 2, 1, 1, 0)
        self.frac_sobel = nn.ModuleList([SinSobel(self.n_filter * (2 ** l), 3, 1) for l in range(self.n_layers, 0, -1)])
        self.frac_conv = nn.ModuleList([MixResidualBlock(self.lat_size, self.n_filter * (2 ** l) * 3, self.n_filter * (2 ** (l - 1)), self.n_filter * (2 ** l)) for l in range(self.n_layers, 0, -1)])
        ...
        self.out_to_lat = LinearResidualBlock(self.n_filter, self.lat_size if not z_out else z_dim)
...
class Decoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, channels, n_filter, n_calls, perception_noise, fire_rate,
        ...
        # self.cell_grads = SinSobel(self.n_filter, list(chain(*[[(2 ** i) + 1] * 2 ** (int(np.log2(image_size)-2) - i) for i in range(1, int(np.log2(image_size)-1), 1)])),
        #                                           list(chain(*[[2 ** (i - 1)] * 2 ** (int(np.log2(image_size)-2) - i) for i in range(1, int(np.log2(image_size)-1), 1)])))

        self.wave_bias = nn.Parameter(torch.nn.init.orthogonal_(torch.empty(1, self.n_filter * 64, 1, 1).repeat(1, 1, self.image_size, self.image_size)))
        self.wave_bias_transform = DynaConv(self.lat_size, self.n_filter * 64, self.n_filter, lat_factor=1)

        # self.wave_to_cell = DynaConv(self.lat_size, self.n_filter * 2, self.n_filter, lat_factor=1)
        # self.grad_to_cell = DynaConv(self.lat_size, self.cell_grads.c_factor * self.n_filter, self.n_filter, lat_factor=1)
        self.cell_to_cell = DynaResidualBlock(self.lat_size, self.n_filter * 3, self.n_filter * (2 if self.gated else 1), self.n_filter * 2, lat_factor=2)
    ...
    def forward(self, lat, ca_init=None, seed_n=0):
        ...
        wave_bias_flat = self.wave_bias_transform(self.wave_bias.repeat(batch_size, 1, 1, 1), dyna_lat)
        ...
        # wave_enc = self.wave_to_cell(wave_bias, dyna_lat)
        ...
        for c in range(self.n_calls):
            ...
            out_new = torch.cat([out_new, wave_bias], dim=1)
        ...
        return out, out_embs, out_raw #, wave_biass
