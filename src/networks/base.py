...
class Discriminator(nn.Module):
    def __init__(self, n_labels, lat_size, z_dim, embed_size, **kwargs):
        ...
        self.labs = nn.Sequential(
            ...
            LinearResidualBlock(self.lat_size, self.lat_size, self.lat_size * 4),
            LinearResidualBlock(self.lat_size, n_labels, self.lat_size * 4),
        )
...
class UnconditionalDiscriminator(nn.Module):
    def __init__(self, lat_size, **kwargs):
        ...
        self.labs = nn.Sequential(
            nn.Linear(lat_size, self.lat_size),
            LinearResidualBlock(self.lat_size, self.lat_size, self.lat_size * 4),
            LinearResidualBlock(self.lat_size, 1, self.lat_size * 4),
        )
