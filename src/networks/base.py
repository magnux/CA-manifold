...
from src.layers.ensemblelinear import EnsembleLinear
...
class Discriminator(nn.Module):
    def __init__(self, n_labels, lat_size, embed_size, **kwargs):
        ...
        self.lat_to_score = EnsembleLinear(self.lat_size, n_labels, bias=False)
...
class Generator(nn.Module):
    def __init__(self, n_labels, lat_size, z_dim, embed_size, norm_z=False, **kwargs):
        ...
        self.register_buffer('embedding_mat', torch.eye(n_labels))
        self.exp_yembed = EnsembleLinear(n_labels, self.embed_size, bias=False)
        self.z_to_lat = EnsembleLinear(self.z_dim + self.embed_size, self.lat_size, bias=False)
...
class LabsEncoder(nn.Module):
    def __init__(self, n_labels, lat_size, embed_size, **kwargs):
        ...
        self.yembed_to_lat = nn.Sequential(
            EnsembleLinear(n_labels, self.embed_size, bias=False),
            EnsembleLinear(self.embed_size, lat_size, bias=False)
        )
...
class UnconditionalDiscriminator(nn.Module):
    def __init__(self, lat_size, **kwargs):
        super().__init__()
        self.lat_size = lat_size
        self.lat_to_score = EnsembleLinear(self.lat_size, 1, bias=False)
...
class UnconditionalGenerator(nn.Module):
    def __init__(self, lat_size, z_dim, norm_z=False, **kwargs):
        ...
        self.z_to_lat = EnsembleLinear(self.z_dim, self.lat_size, bias=False)
