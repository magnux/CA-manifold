...
class Generator(nn.Module):
    def __init__(self, n_labels, lat_size, z_dim, embed_size, n_calls=4, norm_z=False, **kwargs):
        ...
        self.fhidden = lat_size if lat_size > 3 else 512
        self.z_dim = z_dim
        self.embed_size = max(int(embed_size ** 0.5), n_labels * 2)
        self.norm_z = norm_z
        ...
        self.z_to_lat = DynaLinearResidualBlock(self.embed_size, self.z_dim, self.lat_size, bias=False)
    ...
    def forward(self, z, y):
        ...
        lat = lat + 0.1 * self.z_to_lat(z, yembed)
