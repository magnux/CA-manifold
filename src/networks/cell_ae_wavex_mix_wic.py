...
from src.layers.mixresidualblockc import MixResidualBlockC
from src.layers.dynaresidualblockc import DynaResidualBlockC
...
class InjectedEncoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, ds_size, channels, n_filter, z_out=False, z_dim=0, **kwargs):
        ...
        self.frac_sobel = nn.ModuleList([SinSobel(self.n_filter * (2 ** l), 3, 1) for l in range(self.n_layers)])
        self.frac_conv = nn.ModuleList([MixResidualBlockC(self.lat_size, self.n_filter * (2 ** l) * 3, self.n_filter * (2 ** (l + 1)), self.n_filter * (2 ** l)) for l in range(self.n_layers)])
        ...
        self.out_to_lat = LinearResidualBlockC(self.n_filter * (2 ** self.n_layers) * 4, self.lat_size if not z_out else z_dim)
    ...
    def forward(self, x, inj_lat=None):
        ...
        out_embs_raw.append(out)
        lat = self.out_to_lat(out.reshape(batch_size, self.n_filter * (2 ** self.n_layers) * 4))
...
class Decoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, channels, n_filter, n_calls, perception_noise, fire_rate,
                 skip_fire=False, log_mix_out=False, causal=False, gated=True, env_feedback=False, multi_cut=True, ce_out=False, n_seed=1, **kwargs):
        super().__init__()
        ...
        # self.wave_canvases = nn.ParameterList([
        #     nn.Parameter(torch.nn.init.orthogonal_(torch.empty(1, self.n_filter * (2 ** (self.n_calls - l)), 1, 1)).repeat(1, 1, self.image_size, self.image_size)) for l in range(0, self.n_calls)
        # ])
        self.wave_inits = nn.ParameterList([
            nn.Parameter(torch.nn.init.orthogonal_(torch.empty(1, self.n_filter * 2 * (2 ** l) * 3, 1, 1)).repeat(1, 1, self.image_size // (2 ** l), self.image_size // (2 ** l))) for l in range(self.n_layers)
        ])
        self.wave_init_l_raw = LinearResidualBlockC(self.lat_size, self.n_filter * (2 ** (self.n_layers + 1)) * 4)
        self.wave_init_l = LinearResidualBlockC(self.n_filter * (2 ** (self.n_layers + 1)) * 4, self.n_filter * 4)
        self.wave_convs = nn.ModuleList([
            MixResidualBlockC(self.lat_size, self.n_filter * 2 * (2 ** l) * 3, self.n_filter, self.n_filter * 4, 2, 3, 1, 1) for l in range(self.n_layers)
        ])
        # self.wave_bias = nn.Parameter(torch.nn.init.orthogonal_(torch.empty(self.n_seed, self.n_filter * np.sum([2 ** l for l in range(self.n_layers + 1)]), 1, 1).repeat(1, 1, self.image_size, self.image_size)))
        # self.wave_irm = IRMConv(self.n_filter * np.sum([2 ** l for l in range(self.n_layers + 1)]))
        self.wave_bias_transform = DynaResidualBlockC(self.lat_size, self.n_filter * (self.n_layers + 1), (self.n_filter * self.cell_grads.c_factor) // 2, self.n_filter * self.cell_grads.c_factor)

        self.cell_to_cell = DynaResidualBlockC(self.lat_size, self.n_filter * self.cell_grads.c_factor * 2, self.n_filter * (2 if self.gated else 1), self.n_filter * 2, lat_factor=2)
    ...
    def forward(self, lat, ca_init=None, seed_n=0):
        ...
        waves = []
        waves_raw = []
        for l, (wave_init, wave_conv) in enumerate(zip(self.wave_inits, self.wave_convs)):
            wave_init_r = wave_init.repeat(batch_size, 1, 1, 1)
            waves_raw.append(wave_init_r)
            wave = wave_conv(wave_init_r, lat)
            if l > 0:
                wave = F.interpolate(wave, size=self.image_size, mode='bilinear', align_corners=False)
            waves.append(wave)

        wave_init_l_raw = self.wave_init_l_raw(lat)
        waves_raw.append(wave_init_l_raw.reshape(batch_size, self.n_filter * (2 ** (self.n_layers + 1)), 2, 2))
        wave_init_l = self.wave_init_l(wave_init_l_raw).reshape(batch_size, self.n_filter, 2, 2)
        wave_init_l = F.interpolate(wave_init_l, size=self.image_size, mode='bilinear', align_corners=False)
        waves.append(wave_init_l)
        waves_flat = torch.cat(waves, 1)

        waves_flat = self.wave_bias_transform(waves_flat, lat)
        waves = complex_wave(waves_flat)
        ...
        for c in range(self.n_calls):
            ...
            out_new = torch.cat([cell_grads, waves], dim=1)
        ...
        return out, out_embs, waves_raw #, wave_bias
