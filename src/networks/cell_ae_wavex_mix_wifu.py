...
from src.layers.complexwave import complex_wave, complex_to_real
...
class Decoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, channels, n_filter, n_calls, perception_noise, fire_rate,
        ...
        self.wave_inits = nn.ParameterList([
            nn.Parameter(torch.nn.init.orthogonal_(torch.empty(1, self.n_filter * max(l, 1), 1, 1)).repeat(1, 1, (2 ** zoom_factor * self.image_size) // 2 ** (l - 1), (2 ** zoom_factor * self.image_size) // 2 ** (l - 1)))
        for l in range(self.n_layers + 1, 0, -1)])

        self.wave_grads = nn.ModuleList([
            SinSobel(self.n_filter * max(l, 1), 3, 1)
        for l in range(self.n_layers + 1, 0, -1)])

        self.wave_convs = nn.ModuleList([
            DynaResidualBlock(self.lat_size, self.n_filter * max(l, 1) * 3, self.n_filter, self.n_filter * max(l, 1), lat_factor=(self.lat_size ** 0.9) / self.lat_size)
        for l in range(self.n_layers + 1, 0, -1)])

        self.register_buffer('call_emb', torch.eye(self.n_calls).reshape(1, self.n_calls, self.n_calls))
        self.call_enc = nn.Linear(self.n_calls, int(self.lat_size ** 0.5))

        self.cell_to_wave = nn.ModuleList([
            DynaResidualBlock(self.lat_size + int(self.lat_size ** 0.5), self.n_filter * self.cell_grads.c_factor, self.n_filter, self.n_filter * 2, lat_factor=(self.lat_size ** 0.9) / self.lat_size)
        for _ in range(self.n_layers)])

        self.cell_to_cell = DynaResidualBlock(self.lat_size + int(self.lat_size ** 0.5), self.n_filter * 5, self.n_filter * (2 if self.gated else 1), self.n_filter * 2 * (2 if self.gated else 1))
    ...
    def forward(self, lat, img_init=None, noise_inits=None, call_init=None, call_end=None):
        ...
        if noise_inits is None:
            noise_inits = [torch.randn(batch_size, self.out_chan, self.image_size, self.image_size, device=lat.device).clamp_(-1, 1) for _ in range(call_end - call_init)]

        waves = []
        long_wave = None
        for l, (wave_init, wave_grad, wave_conv) in enumerate(zip(self.wave_inits, self.wave_grads, self.wave_convs)):
            wave = wave_init.repeat(batch_size, 1, 1, 1)
            wave = wave_grad(wave)
            wave = wave_conv(wave, lat)
            if wave.shape[2] != (2 ** self.zoom_factor * self.image_size):
                wave = F.interpolate(wave, size=(2 ** self.zoom_factor * self.image_size), mode='bilinear', align_corners=False)
            if l == 0:
                long_wave = complex_wave(wave, return_real=False)
            else:
                long_wave = long_wave * complex_wave(wave, return_real=False)
            waves.append(wave)

        long_wave = complex_to_real(long_wave)
        ...
        for c in call_range:
            ...
            call_lat = torch.cat([lat, self.call_enc(self.call_emb[:, c, :]).repeat(batch_size, 1)], dim=1)

            short_wave = None
            for l, wave_conv in enumerate(self.cell_to_wave):
                wave = cell_in
                if l > 0:
                    wave = F.interpolate(wave, size=self.image_size // 2 ** l, mode='bilinear', align_corners=False)
                wave = self.cell_grads(wave)
                wave = wave_conv(wave, call_lat)
                if l > 0:
                    wave = F.interpolate(wave, size=2 ** self.zoom_factor * self.image_size, mode='bilinear', align_corners=False)
                if l == 0:
                    short_wave = complex_wave(wave, return_real=False)
                else:
                    short_wave = short_wave * complex_wave(wave, return_real=False)

            short_wave = complex_to_real(short_wave)

            cell_in = torch.cat([cell_in, short_wave, long_wave], dim=1)

            cell_out = self.cell_to_cell(cell_in, call_lat)
