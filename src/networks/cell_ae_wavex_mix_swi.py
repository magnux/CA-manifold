...
class Decoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, channels, n_filter, n_calls, perception_noise, fire_rate,
        ...
        self.n_filter = n_filter
        self.lat_size = lat_size
        self.n_calls = int(np.log2(image_size)) - 1
        ...
        self.wave_bias = nn.ParameterList([
            nn.Parameter(torch.nn.init.orthogonal_(torch.empty(self.n_seed, self.n_filter * (2 ** l), 1, 1).repeat(1, 1, 2 ** (self.n_calls - l + 2), 2 ** (self.n_calls - l + 2))))
            for l in range(self.n_calls + 1, 1, -1)])
        # self.wave_irm = nn.ModuleList([IRMConv(self.n_filter * (2 ** l)) for l in range(self.n_calls + 1, 1, -1)])
        self.wave_bias_transform = DynaConv(self.lat_size, self.n_filter * np.sum([(2 ** l) for l in range(self.n_calls + 1, 1, -1)]), self.n_filter)

        self.cell_to_cell = MixResidualBlock(self.lat_size, self.n_filter * (self.cell_grads.c_factor + 2), self.n_filter * (2 if self.gated else 1), self.n_filter * 2, lat_factor=2)
    ...
    def forward(self, lat, ca_init=None, seed_n=0):
        ...
        for c in range(self.n_calls):
            if isinstance(seed_n, tuple):
                wave_bias_flat = self.wave_bias[c][seed_n[0]:seed_n[1], ...].mean(dim=0, keepdim=True)
            elif isinstance(seed_n, list):
                wave_bias_flat = self.wave_bias[c][seed_n, ...].mean(dim=0, keepdim=True)
            else:
                wave_bias_flat = self.wave_bias[c][seed_n:seed_n + 1, ...]

            # wave_bias_flat = self.wave_irm[c](wave_bias_flat)
            if wave_bias_flat.shape[2] != self.image_size:
                wave_bias_flat = F.interpolate(wave_bias_flat, size=self.image_size, mode='bilinear', align_corners=False)
            wave_biases.append(wave_bias_flat)

        wave_bias_flat = torch.cat(wave_biases, 1)
