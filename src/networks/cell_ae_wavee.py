...
class InjectedEncoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, channels, n_filter, n_calls,
                 multi_cut=True, z_out=False, z_dim=0, ce_in=False, **kwargs):
        super().__init__()
        self.injected = True
        ...
        self.frac_grads = RandGrads(self.n_filter, list(chain(*[[(2 ** i) + 1] * 2 ** (int(np.log2(image_size)-2) - i) for i in range(1, int(np.log2(image_size)-1), 1)])),
                                                   list(chain(*[[2 ** (i - 1)] * 2 ** (int(np.log2(image_size)-2) - i) for i in range(1, int(np.log2(image_size)-1), 1)])))

        self.frac_conv = MixResidualBlock(self.lat_size, self.n_filter * self.frac_grads.c_factor, self.n_filter, lat_factor=4)

        self.out_conv = nn.Conv2d(self.n_filter, sum(self.split_sizes), 1, 1, 0)
        self.out_to_lat = nn.Sequential(
            LinearResidualBlock(sum(self.conv_state_size) + self.lat_size, self.lat_size, self.lat_size * 2),
    ...
    def forward(self, x, inj_lat=None):
        ...
        self.frac_grads.call_c = 0
        lat = torch.zeros(batch_size, self.lat_size, device=x.device)
        for c in range(self.n_calls):
            out_new = out

            out_new = self.frac_grads(out_new)
            out_new = F.instance_norm(out_new)
            out_new = self.frac_conv(out_new, inj_lat)

            conv_state = self.out_conv(out_new)
            if self.multi_cut:
