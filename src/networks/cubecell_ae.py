...
class InjectedEncoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, channels, n_filter, n_calls, perception_noise, fire_rate,
                 skip_fire=False, causal=False, gated=False, env_feedback=False, multi_cut=True, z_out=False, z_dim=0, auto_reg=True, conv_irm=False, ce_in=False, **kwargs):
        super().__init__()
        self.injected = True
...
class Decoder(nn.Module):
    def __init__(self, n_labels, lat_size, image_size, channels, n_filter, n_calls, perception_noise, fire_rate,
                 skip_fire=False, log_mix_out=False, causal=False, gated=False, env_feedback=False, redec_ap=False, auto_reg=True, conv_irm=False, ce_out=False, n_seed=1, **kwargs):
        super().__init__()
        ...
        self.seed = nn.Parameter(torch.nn.init.orthogonal_(torch.empty(n_seed, self.n_filter)).unsqueeze(2).unsqueeze(3).unsqueeze(4).repeat(1, 1, self.cube_size, self.cube_size, self.cube_size))
        if self.conv_irm:
            self.frac_irm = IRMConv(self.n_filter, dim=3)
        self.frac_sobel = SinSobel(self.n_filter, [(2 ** i) + 1 for i in range(1, int(np.log2(self.cube_size)-1), 1)],
