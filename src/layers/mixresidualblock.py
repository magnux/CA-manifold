...
class MixResidualBlock(nn.Module):
    def __init__(self, lat_size, fin, fout, fhidden=None, dim=2, kernel_size=1, stride=1, padding=0, groups=1, lat_factor=1, n_mix=8):
        ...
        self.k_in_mix = nn.Parameter(torch.nn.init.xavier_normal_(torch.empty(*([1, n_mix, self.fhidden, self.fin // groups] + [kernel_size for _ in range(dim)]))))
        self.k_mid_mix = nn.Parameter(torch.nn.init.xavier_normal_(torch.empty(*([1, n_mix, self.fhidden, self.fhidden // groups] + [kernel_size for _ in range(dim)]))))
        self.k_out_mix = nn.Parameter(torch.nn.init.xavier_normal_(torch.empty(*([1, n_mix, self.fout, self.fhidden // groups] + [kernel_size for _ in range(dim)]))))
        ...
        self.b_in_mix = nn.Parameter(torch.nn.init.xavier_normal_(torch.empty(*([1, n_mix, self.fhidden] + [1 for _ in range(dim)]))))
        self.b_mid_mix = nn.Parameter(torch.nn.init.xavier_normal_(torch.empty(*([1, n_mix, self.fhidden] + [1 for _ in range(dim)]))))
    ...
    def forward(self, x, lat):
        ...
        if self.prev_lat is None or self.prev_lat.data_ptr() != lat.data_ptr():
            ...
            self.k_in = (self.k_in_mix * ks).sum(1).reshape([batch_size * self.fhidden, self.fin // self.groups] + self.kernel_size)
            self.k_mid = (self.k_mid_mix * ks).sum(1).reshape([batch_size * self.fhidden, self.fhidden // self.groups] + self.kernel_size)
            self.k_out = (self.k_out_mix * ks).sum(1).reshape([batch_size * self.fout, self.fhidden // self.groups] + self.kernel_size)
            ...
            self.b_in = (self.b_in_mix * ks).sum(1).reshape([1, batch_size * self.fhidden] + [1 for _ in range(self.dim)])
            self.b_mid = (self.b_mid_mix * ks).sum(1).reshape([1, batch_size * self.fhidden] + [1 for _ in range(self.dim)])
