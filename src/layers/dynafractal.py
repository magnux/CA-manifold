...
class DynaFractal(nn.Module):
    def __init__(self, lat_size, fin, fout, n_calls=4, bias=True):
        super(DynaFractal, self).__init__()
    ...
    def forward(self, x, lat):
        ...
        for c in range(self.n_calls):
            x_new_f = torch.bmm(x_new, self.wf) + self.bf
            x_new = x_new + 0.1 * F.relu(x_new_f)
