...
class DynaConv(nn.Module):
    ...
    def forward(self, x, lat):
        ...
        if self.prev_lat is None or self.prev_lat.data_ptr() != lat.data_ptr():
            ...
            self.k = k.view([batch_size, self.fout, self.fin] + self.kernel_size).reshape([batch_size * self.fout, self.fin] + self.kernel_size)
        ...
        x_new = self.f_conv(x_new, self.k, groups=batch_size) + self.b
