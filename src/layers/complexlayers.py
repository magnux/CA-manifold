...
from src.layers.complexfunctions import complex_relu, complex_max_pool2d
from src.layers.complexfunctions import complex_dropout, complex_dropout2d
...
class ComplexConvTranspose2d(Module):
    ...
    def forward(self, input_r, input_i):
        return self.conv_tran_r(input_r) - self.conv_tran_i(input_i), self.conv_tran_r(input_i) + self.conv_tran_i(input_r)
...
class ComplexBatchNorm1d(_ComplexBatchNorm):
    ...
    def forward(self, input_r, input_i):
        ...
        if self.affine:
            input_r, input_i = self.weight[None, :, 0] * input_r + self.weight[None, :, 2] * input_i +  self.bias[None, :, 0], \
                               self.weight[None, :, 2] * input_r + self.weight[None, :, 1] * input_i + self.bias[None, :, 1]

        del Crr, Cri, Cii, Rrr, Rii, Rri, det, s, t
        return input_r, input_i


class ComplexInstanceNorm2d(_ComplexBatchNorm):
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True, track_running_stats=True):
        super(ComplexInstanceNorm2d, self).__init__(num_features, eps, momentum, affine, track_running_stats)
    ...
    def forward(self, input_r, input_i):
        ...
        if self.training:
            ...
            with torch.no_grad():
                self.running_covar[:, 0] = exponential_average_factor * Crr.mean(0) * n / (n - 1) \
                                           + (1 - exponential_average_factor) * self.running_covar[:, 0]

                self.running_covar[:, 1] = exponential_average_factor * Cii.mean(0) * n / (n - 1) \
                                           + (1 - exponential_average_factor) * self.running_covar[:, 1]
