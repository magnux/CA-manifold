...
def build_optimizer(network, config):
    ...
    # Optimizers
    if optimizer == 'adam':
        optimizer = optim.Adam(params, lr=lr, betas=(0.9, 0.99), weight_decay=lr * 1e-2, amsgrad=True)
    elif optimizer == 'adamp':
        optimizer = AdamP(params, lr=lr, betas=(0.9, 0.99), weight_decay=lr * 1e-2, nesterov=True)
    elif optimizer == 'sgd':
