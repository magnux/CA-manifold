...
model_manager = ModelManager('gaxr', networks_dict, config)
                ...
                with model_manager.on_batch():
                    ...
                    with model_manager.on_step(['dis_encoder', 'discriminator', 'decoder', 'generator']) as nets_to_train:
                        for _ in range(batch_split):
                            ...
                            loss_gen_unb = (1 / batch_split) * compute_gan_loss(labs_dec, (it+1) % 2)
...
d_reg_every_mean_next_enc = d_reg_every_mean
d_reg_every_mean_next_dec = d_reg_every_mean
d_reg_param_mean_enc = model_manager.log_manager.get_last('regs', 'd_reg_param_mean_enc', 1 / d_reg_param)
d_reg_param_mean_dec = model_manager.log_manager.get_last('regs', 'd_reg_param_mean_dec', 1 / d_reg_param)
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                if d_reg_every_mean > 0 and model_manager.it % d_reg_every_mean == 0:
                    d_reg_factor_enc = (d_reg_every_mean_next_enc - (model_manager.it % d_reg_every_mean_next_enc)) * (1 / d_reg_param_mean_enc)
                    d_reg_factor_dec = (d_reg_every_mean_next_enc - (model_manager.it % d_reg_every_mean_next_enc)) * (1 / d_reg_param_mean_dec)
                else:
                ...
                with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        # labs_enc = discriminator(lat_top_dec)
                        #
                        # loss_dis_unb = (1 / batch_split) * compute_gan_loss(labs_enc, model_manager.it % 2)
                        # model_manager.loss_backward(loss_dis_unb, nets_to_train)
                        # loss_dis_unb_sum += loss_dis_unb.item()
                    ...
                    if d_reg_every_mean > 0 and model_manager.it % d_reg_every_mean == 0:
                        d_reg_every_mean_enc = d_reg_every_mean_next_enc
                        d_reg_every_mean_next_enc, d_reg_param_mean_enc = update_reg_params(d_reg_every_mean_next_enc, d_reg_every,
                                                                                            d_reg_param_mean_enc, 1 / d_reg_param,
                                                                                            reg_dis_enc_sum, reg_dis_dec_sum, loss_dis_enc_sum)
                        d_reg_every_mean_dec = d_reg_every_mean_next_dec
                        d_reg_every_mean_next_dec, d_reg_param_mean_dec = update_reg_params(d_reg_every_mean_next_dec, d_reg_every,
                                                                                            d_reg_param_mean_dec, 1 / d_reg_param,
                                                                                            reg_dis_dec_sum, reg_dis_enc_sum, loss_dis_dec_sum)
                        d_reg_every_mean = min(d_reg_every_mean_enc, d_reg_every_mean_dec)
                ...
                stream_images(images_gen, config_name + '/gaxr', config['training']['out_dir'] + '/gaxr')
                ...
                model_manager.log_scalar('regs',  'd_reg_param_mean_enc',  d_reg_param_mean_enc)
                model_manager.log_scalar('regs',  'd_reg_param_mean_dec',  d_reg_param_mean_dec)
