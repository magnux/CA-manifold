...
parser = argparse.ArgumentParser(description='Train a RAGEX')
...
model_manager = ModelManager('ragex', networks_dict, config)
    ...
    for epoch in range(model_manager.start_epoch, config['training']['n_epochs'] // 8):
        with model_manager.on_epoch(epoch):
            ...
            for batch in t:
                ...
                stream_images(images_dec, config_name + '/ragex_pretrain', config['training']['out_dir'] + '/ragex_pretrain')
    ...
    with model_manager.on_epoch(epoch):
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                reg_mean = reg_dis_target
                while reg_mean >= reg_dis_target:
                    reg_dis_enc_sum, reg_dis_dec_sum = 0, 0
                    loss_gen_enc_sum, loss_gen_dec_sum = 0, 0
                    ...
                    with model_manager.on_step(['decoder', 'generator']) as nets_to_train:
                        ...
                        reg_mean = 0.5 * reg_mean + 0.25 * (reg_dis_enc_sum + reg_dis_dec_sum)
                ...
                stream_images(images_gen, config_name + '/ragex', config['training']['out_dir'] + '/ragex')
