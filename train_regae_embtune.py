...
# Networks
networks_dict = {
    'clean_encoder': {'class': config['network']['class'], 'sub_class': 'NInjectedEncoder'},
    'noisy_encoder': {'class': config['network']['class'], 'sub_class': 'NInjectedEncoder'},
...
model_manager = ModelManager('regae_embtune', networks_dict, config)  #, to_avg=['encoder', 'decoder'])
...
# # Distributions
# zdist_mu, zdist_cov = load_multigauss_params(config['training']['out_dir'] + '/regae_embtune', lat_size, device=device)
# zdist_cov_inv = None
...
def sample_regae(images_or_lat, labels, prev_perm=None, next_perm=None, dec_only=False):
    ...
    if not dec_only:
        ...
        if injected_encoder:
            lat_enc, out_embs, _ = clean_encoder(images, labels)
        else:
            lat_enc, out_embs, _ = clean_encoder(images)
    ...
    for call_idx in range(n_calls):
        ...
        images_init = (1 - nf) * images_dec + nf * torch.group_norm(torch.randn((batch_split_size, channels, image_size, image_size), device=device), 1)

        nlat_enc, _, _ = noisy_encoder(images_init, (labels[next_perm] if fussion_progress > 0.5 else labels[prev_perm]) if prev_perm is not None else labels)
        # letters = letter_encoder(nlat_enc)
        # nlat_dec = letter_decoder(letters)
        images_dec = 0.5 * (decoder(images_init, lat_enc, None) + decoder(images_init, None, nlat_enc))
    ...
    with model_manager.on_epoch():
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                loss_cond_sum, loss_emb_cond_sum = 0, 0
                loss_uncond_sum, loss_emb_uncond_sum = 0, 0
                ...
                with model_manager.on_step(['clean_encoder', 'noisy_encoder', 'decoder']) as nets_to_train:  # , 'letter_encoder', 'letter_decoder'
                    ...
                    for b in range(batch_mult * 2):
                        ...
                        with torch.no_grad():
                            ...
                            #     noise_init = sample_regae(lat_dec, labels, dec_only=True)
                            # else:
                        ...
                        if b % 2 == 1:
                            noise_out = decoder(images_init, lat_enc, None, rev_switch)
                            loss_cond = (1 / batch_mult) * 2 * F.mse_loss(noise_out, noise_init)

                            model_manager.loss_backward(loss_cond, nets_to_train)
                            loss_cond_sum += loss_cond.item()
                        else:
                            images_out = decoder(images_init, lat_enc, None, None)
                            lat_reenc, _, _ = clean_encoder(images_out, labels)
                            # reletters = letter_encoder(lat_reenc)
                            # loss_emb_cond = (1 / batch_mult) * (F.cross_entropy(reletters, letters.detach()) / F.cross_entropy(reletters, letters[train_perm].detach()))
                            loss_emb_cond = (1 / batch_mult) * F.mse_loss(lat_reenc, lat_enc.detach())

                            model_manager.loss_backward(loss_emb_cond, nets_to_train)
                            loss_emb_cond_sum += loss_emb_cond.item()

                        with torch.no_grad():
                            # if model_manager.it % 2 == 1:
                            #     noise_init = sample_regae(None, labels, dec_only=True)
                            # else:
                            noise_init = torch.group_norm(torch.randn_like(images), 1)
                            nf = noise_factor(call_idcs, ministep)[:, None, None, None]
                            images_init = (1 - nf) * images + nf * noise_init
                        ...
                        nlat_enc, _, _ = noisy_encoder(images_init, labels)
                        ...
                        if b % 2 == 1:
                            noise_out = decoder(images_init, None, nlat_enc, rev_switch)
                            loss_uncond = (1 / batch_mult) * 2 * F.mse_loss(noise_out, noise_init)

                            model_manager.loss_backward(loss_uncond, nets_to_train)
                            loss_uncond_sum += loss_uncond.item()
                        else:
                            images_out = decoder(images_init, None, nlat_enc, None)
                            nlat_reenc, _, _ = noisy_encoder(images_out, labels)
                            # reletters = letter_encoder(nlat_reenc)
                            # loss_emb_uncond = (1 / batch_mult) * (F.cross_entropy(reletters, letters.detach()) / F.cross_entropy(reletters, letters[train_perm].detach()))
                            loss_emb_uncond = (1 / batch_mult) * F.mse_loss(nlat_reenc, nlat_enc.detach())

                            model_manager.loss_backward(loss_emb_uncond, nets_to_train)
                            loss_emb_uncond_sum += loss_emb_uncond.item()

                model_manager.set_err(loss_cond_sum + loss_emb_cond_sum + loss_uncond_sum + loss_emb_uncond_sum)
                ...
                if config['training']['stream_every'] > 0 and (model_manager.it % config['training']['stream_every']) == 0:
                    ...
                    stream_images(images_dec, config_name + '/regae_embtune', config['training']['out_dir'] + '/regae_embtune')
                ...
                # Print progress
                running_loss[batch % window_size] = loss_cond_sum + loss_emb_cond_sum + loss_uncond_sum + loss_emb_uncond_sum
                ...
                model_manager.log_scalar('losses', 'loss_cond', loss_cond_sum)
                model_manager.log_scalar('losses', 'loss_emb_cond', loss_emb_cond_sum)
                model_manager.log_scalar('losses', 'loss_uncond', loss_uncond_sum)
                model_manager.log_scalar('losses', 'loss_emb_uncond', loss_emb_uncond_sum)

    # save_multigauss_params(zdist_mu, zdist_cov, config['training']['out_dir'] + '/regae_embtune')
