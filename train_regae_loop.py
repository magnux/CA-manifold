...
model_manager = ModelManager('regae_loop', networks_dict, config)  #, to_avg=['encoder', 'decoder'])
...
# # Distributions
# zdist_mu, zdist_cov = load_multigauss_params(config['training']['out_dir'] + '/regae_loop', lat_size, device=device)
# zdist_cov_inv = None
...
def sample_regae(images_or_lat, labels, prev_perm=None, next_perm=None, dec_only=False):
    ...
    for call_idx in range(n_calls):
        ...
        if call_idx < n_calls - 1:
            images_dec_l.append(decoder(lat_dec, images_init, labels))

        if call_idx > 0:
            images_dec_l.append(decoder(None, images_init, labels))

        images_dec = torch.stack(images_dec_l, -1).mean(-1)
...
for _ in range(model_manager.epoch, n_epochs):
            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['encoder', 'decoder'] + (['letter_encoder', 'letter_decoder'] if letter_encoding else [])) as nets_to_train:
                    ...
                    for b in range(batch_mult * 2):
                        ...
                        images_out = decoder(lat_dec, images_init, labels, rev_switch if b % 2 == 1 else None)
                        ...
                        images_out = decoder(None, images_init, labels, rev_switch if b % 2 == 1 else None)
                ...
                if config['training']['stream_every'] > 0 and (model_manager.it % config['training']['stream_every']) == 0:
                    ...
                    stream_images(images_dec, config_name + '/regae_loop', config['training']['out_dir'] + '/regae_loop')
    ...
    # save_multigauss_params(zdist_mu, zdist_cov, config['training']['out_dir'] + '/regae_loop')
