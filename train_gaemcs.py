...
config['network']['kwargs']['n_seed'] = n_labels
...
model_manager = ModelManager('gaemcs', networks_dict, config)
    ...
    for epoch in range(model_manager.start_epoch, n_epochs // 8):
        with model_manager.on_epoch(epoch):
            ...
            for batch in t:
                ...
                stream_images(images_dec, config_name + '/gaemcs_pretrain', config['training']['out_dir'] + '/gaemcs_pretrain')
...
for epoch in range(model_manager.start_epoch, n_epochs):
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                with torch.no_grad():
                    ...
                    images_gen, _, _ = decoder(lat_gen, seed_n=labels_test)
                    # images_regen, _, _ = decoder(lat_gen, seed_n=(0, n_seed))
                    # images_gen = torch.cat([images_gen, images_regen], dim=3)

                stream_images(images_gen, config_name + '/gaemcs', config['training']['out_dir'] + '/gaemcs')
    ...
    with torch.no_grad():
        ...
        if config['training']['sample_every'] > 0 and ((epoch + 1) % config['training']['sample_every']) == 0:
            ...
            z_enc, _, _ = encoder(images, labels)
            lat_enc = generator(z_enc)
            images_dec, _, _ = decoder(lat_enc, seed_n=labels)
            ...
            for lab in range(config['training']['sample_labels']):
                ...
                #     fixed_lab[:, lab] = 1
                lat_gen = generator(z_test)
                images_gen, _, _ = decoder(lat_gen, seed_n=lab)
                # images_regen, _, _ = decoder(lat_gen, seed_n=(0, n_seed))
                # images_gen = torch.cat([images_gen, images_regen], dim=3)
