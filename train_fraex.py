...
from src.distributions import get_ydist, get_zdist, load_multigauss_params, save_multigauss_params, update_multigauss_params
...
parser = argparse.ArgumentParser(description='Train a FRAEX')
...
model_manager = ModelManager('fraex', networks_dict, config)
...
for epoch in range(model_manager.start_epoch, config['training']['n_epochs']):
    ...
    # save_multigauss_params(zdist_mu, zdist_cov, join(config['training']['out_dir'], 'fraex'))
