...
n_seed = 4
config['network']['kwargs']['n_seed'] = (n_labels * n_seed) + 1
seed_idx = torch.arange((n_labels * n_seed) + 1).reshape(n_labels + 1, n_seed)
        ...
        with model_manager.on_epoch(epoch):
            ...
            for batch in t:
                with model_manager.on_batch():
                    ...
                    with model_manager.on_step(['encoder', 'decoder', 'generator']) as nets_to_train:
                        for _ in range(batch_split):
                            ...
                            images_dec, _, _ = decoder(lat_enc, seed=seed_idx[torch.ones_like(labels) * n_labels + 1, it % n_seed].tolist())
                ...
                with torch.no_grad():
                    ...
                    lat_enc = generator(z_enc)
                    images_dec, _, _ = decoder(lat_enc, seed=seed_idx[torch.ones_like(labels) * n_labels + 1, :].tolist())
                ...
                with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        with torch.no_grad():
                            ...
                            images_redec, _, _ = decoder(lat_gen, seed_n=seed_idx[labels, it % n_seed].tolist())
