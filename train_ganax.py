...
parser = argparse.ArgumentParser(description='Train a GANAX')
...
networks_dict = {
    ...
    'dis_generator': {'class': 'base', 'sub_class': 'LabsImgEncoder'},
...
model_manager = ModelManager('ganax', networks_dict, config)
decoder = model_manager.get_network('decoder')
generator = model_manager.get_network('generator')
dis_generator = model_manager.get_network('dis_generator')
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                def train_d(loss_dis_enc_sum, labs_dis_enc_sign, reg_dis_enc_sum,
                            loss_dis_dec_sum, labs_dis_dec_sign, reg_dis_dec_sum, g_factor_enc, g_factor_dec,
                    ...
                    with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                        ...
                        for _ in range(batch_mult):
                            ...
                            lat_dis_enc = dis_generator(images, labels)
                            lat_dis_enc = lat_dis_enc + g_factor_enc * torch.randn_like(lat_dis_enc)
                            ...
                            lat_dis_dec = dis_generator(images_dec, labels)
                            lat_dis_dec = lat_dis_dec + g_factor_enc * torch.randn_like(lat_dis_dec)
                            lat_top_dec, _, _ = dis_encoder(images_dec, lat_dis_dec)
                ...
                def train_g(loss_gen_enc_sum, loss_gen_dec_sum, reg_gen_dec_sum, trainiter, pl_mean_dec):
                    # Generator step
                    with model_manager.on_step(['decoder', 'generator', 'dis_generator']) as nets_to_train:
                    ...
                    return loss_gen_enc_sum, loss_gen_dec_sum, reg_gen_dec_sum, trainiter, pl_mean_dec
                ...
                for turn in train_order:
                    if turn:
                        ...
                        loss_gen_enc_sum, loss_gen_dec_sum, reg_gen_dec_sum, trainiter, pl_mean_dec = train_g(loss_gen_enc_sum, loss_gen_dec_sum, reg_gen_dec_sum, trainiter, pl_mean_dec)
                ...
                stream_images(images_gen, config_name + '/ganax', config['training']['out_dir'] + '/ganax')
