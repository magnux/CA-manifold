            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        loss_dis_enc.register_hook(lambda grad: ((1. - noise_f) * grad) + (noise_f * (torch.randn_like(grad) - grad)))
                        ...
                        loss_dis_dec.register_hook(lambda grad: ((1. - noise_f) * grad) + (noise_f * (torch.randn_like(grad) - grad)))
                ...
                with model_manager.on_step(['encoder', 'decoder', 'generator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        loss_gen_enc.register_hook(lambda grad: ((1. - noise_f) * grad) + (noise_f * (torch.randn_like(grad) - grad)))
                        ...
                        loss_gen_dec.register_hook(lambda grad: ((1. - noise_f) * grad) + (noise_f * (torch.randn_like(grad) - grad)))
