            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        lat_top_enc.register_hook(lambda grad: ((1. - g_factor_enc) * grad) + (g_factor_enc * (1e-2 * torch.rand_like(grad) - grad)))
                        ...
                        lat_top_dec.register_hook(lambda grad: ((1. - g_factor_dec) * grad) + (g_factor_dec * (1e-2 * torch.rand_like(grad) - grad)))
