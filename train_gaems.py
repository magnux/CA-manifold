            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        lat_top_enc.register_hook(lambda grad: ((1. - noise_f) * grad) + (noise_f * (1e-3 * torch.rand_like(grad) - grad)))
                        ...
                        lat_top_dec.register_hook(lambda grad: ((1. - noise_f) * grad) + (noise_f * (1e-3 * torch.rand_like(grad) - grad)))
                    ...
                    if labs_dis_enc_sign > 0:
                        noise_f = np.clip(noise_f + (np.clip(np.sign(labs_dis_enc_sign - 0.2) * 2, -2, 1) * 1e-3), 0., 1.)
                    else:
                        noise_f = np.clip(noise_f + (np.clip(-np.sign(labs_dis_enc_sign - 0.0) * 2, -1, 2) * 1e-3), 0., 1.)
