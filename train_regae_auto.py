...
model_manager = ModelManager('regae', networks_dict, config)  #, to_avg=['encoder', 'decoder'])
...
def noise_factor(call_idx):
        if isinstance(call_idx, int):
            call_idx = torch.ones([]) * call_idx
        return (0.5 * (torch.cos(np.pi * call_idx / n_calls) + 1)) ** 0.69
...
# Distributions
zdist_mu, zdist_cov = load_multigauss_params(config['training']['out_dir'] + '/regae', lat_size, device=device)
zdist_cov_inv = None
...
def sample_regae(images_or_lat, labels, prev_perm=None, next_perm=None, dec_only=False):
    ...
    if not dec_only:
        ...
        if letter_encoding:
            ...
            # Latent space interpolation
            if prev_perm is not None:
                lat_dec_n = (1 - fussion_progress) * lat_dec_n[prev_perm] + fussion_progress * lat_dec_n[next_perm]
    ...
    images_dec = 2 * torch.randn((batch_split_size, channels, image_size, image_size), device=device)
    images_dec = requantize(images_dec)
    images_dec = decoder(lat_dec, images_dec, labels)
    for call_idx in range(1, n_calls):
        images_dec = images_dec + noise_factor(call_idx) * torch.randn_like(images_dec)
        images_dec = requantize(images_dec)
...
call_idcs_test = (torch.arange(batch_size, device=device) % (n_calls - 1)) + 1
...
for _ in range(model_manager.epoch, n_epochs):
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['encoder', 'decoder'] + (['letter_encoder', 'letter_decoder'] if letter_encoding else [])) as nets_to_train:
                    ...
                    for b in range(batch_mult):
                        ...
                        with torch.no_grad():
                            ...
                            noise_init = 2 * torch.randn_like(images)
                            noise_init = requantize(noise_init)

                        noise_init.requires_grad_()

                        images_out = decoder(lat_dec, noise_init, labels)
                        ...
                        with torch.no_grad():
                            images_init = images + noise_factor(call_idcs)[:, None, None, None] * torch.randn_like(images)
                            images_init = requantize(images_init)
                            noise_init = images_init - images

                        images_init.requires_grad_(True)

                        images_out = decoder(None, images_init, labels)
                        noise_out = images_init - images_out

                        loss_denoise = (1 / batch_mult) * F.mse_loss(noise_out, noise_init)
                ...
                if config['training']['stream_every'] > 0 and (model_manager.it % config['training']['stream_every']) == 0:
                    ...
                    stream_images(images_dec, config_name + '/regae', config['training']['out_dir'] + '/regae')
    ...
    save_multigauss_params(zdist_mu, zdist_cov, config['training']['out_dir'] + '/regae')
