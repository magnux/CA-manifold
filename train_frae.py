            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        lat_top_enc, _, _ = dis_encoder(images_dec, labels)
                        labs_enc = discriminator(lat_top_enc)
                        labs_dis_enc_sign += ((1 / batch_mult) * labs_enc.sign().mean()).item()

                        if d_reg_every_mean > 0 and model_manager.it % d_reg_every_mean == 0:
                            reg_dis_enc = (1 / batch_mult) * d_reg_factor * compute_grad_reg(labs_enc, images_dec)
                        ...
                        with torch.no_grad():
                            ...
                            #     images_redec = images_dec
                            # else:
                            #     images_redec, _, _ = redecoder(lat_gen, out_embs[-1])
                        ...
                        # images_mix[batch_split_size//2:, ...].data.copy_(images_redec[batch_split_size//2:, ...])
                ...
                with model_manager.on_step(['encoder', 'decoder', 'redecoder', 'generator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        #     reg_gen_enc, pl_mean_enc = compute_pl_reg(z_enc, images, pl_mean_enc)
                        ...
                        #         images_redec, _, _ = redecoder(lat_enc, corrupt_inits[model_manager.it % 3])
                        #     else:
                        #         images_redec, _, _ = redecoder(lat_enc.detach().clone(), corrupt_inits[model_manager.it % 3].detach().clone())
                        #
                        # lat_top_dec, _, _ = dis_encoder(images_redec, lat_enc.detach().clone())
                        # labs_dec = discriminator(lat_top_dec)
                        #
                        # if g_reg_every > 0 and model_manager.it % g_reg_every == 0:
                        #     reg_gen_dec, pl_mean_dec = compute_pl_reg(images_redec, z_enc, pl_mean_dec)
                        ...
                        #         images_redec, _, _ = redecoder(lat_gen, out_embs[-1])
                        #     else:
                        #         images_redec, _, _ = redecoder(lat_gen.detach().clone(), out_embs[-1].detach().clone())

                        lat_top_dec, _, _ = dis_encoder(images_dec, labels)
                        labs_dec = discriminator(lat_top_dec)

                        if g_reg_every > 0 and model_manager.it % g_reg_every == 0:
                            reg_gen_dec, pl_mean_dec = compute_pl_reg(images_dec, z_gen, pl_mean_dec)
                            reg_gen_dec = (1 / batch_mult) * g_reg_every * reg_gen_dec
                            model_manager.loss_backward(reg_gen_dec, nets_to_train, retain_graph=True)
                            reg_gen_dec_sum += reg_gen_dec.item() / g_reg_every
