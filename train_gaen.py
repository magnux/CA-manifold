    ...
    with model_manager.on_epoch(epoch):
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                loss_gen_reenc_sum, loss_gen_redec_sum = 0, 0
                ...
                with model_manager.on_step(['encoder', 'labs_encoder', 'decoder', 'generator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        if g_reg_every > 0 and it % g_reg_every == 0:
                            if alt_reg:
                                reg_gen_enc = compute_grad_reg(lat_enc, images, 1)
                                reg_gen_enc = (1 / batch_mult) * 0.9 * g_reg_every * 1e-1 * reg_gen_enc
                                ...
                                reg_gen_enc = (1 / batch_mult) * 0.9 * g_reg_every * reg_gen_enc
                                model_manager.loss_backward(reg_gen_enc, nets_to_train, retain_graph=True)
                                reg_gen_enc_sum += reg_gen_enc.item()

                        loss_gen_enc = (1 / batch_mult) * 0.9 * compute_gan_loss(labs_enc, 0)
                        ...
                        if g_reg_every > 0 and it % g_reg_every == (g_reg_every // 2):
                            if alt_reg:
                                reg_gen_dec = compute_grad_reg(images_dec, lat_gen, 1)
                        ...
                        if g_reg_every > 0 and it % g_reg_every == 0:
                            if alt_reg:
                                reg_gen_enc = compute_grad_reg(lat_enc, images_dec, 1)
                                reg_gen_enc = (1 / batch_mult) * 0.1 * g_reg_every * 1e-1 * reg_gen_enc
                                model_manager.loss_backward(reg_gen_enc, nets_to_train, retain_graph=True)
                                reg_gen_enc_sum += reg_gen_enc.item()
                            else:
                                reg_gen_enc, pl_mean_enc = compute_pl_reg(lat_enc, images_dec, pl_mean_enc)
                                reg_gen_enc = (1 / batch_mult) * 0.1 * g_reg_every * reg_gen_enc
                                model_manager.loss_backward(reg_gen_enc, nets_to_train, retain_graph=True)
                                reg_gen_enc_sum += reg_gen_enc.item()

                        loss_gen_reenc = (1 / batch_mult) * 0.1 * compute_gan_loss(labs_enc, 0)
                        model_manager.loss_backward(loss_gen_reenc, nets_to_train)
                        ...
                        if g_reg_every > 0 and it % g_reg_every == (g_reg_every // 2):
                            if alt_reg:
                                reg_gen_dec = compute_grad_reg(images_dec, lat_enc, 1)
                ...
                running_loss_gen[batch % window_size] = loss_gen_enc_sum + loss_gen_dec_sum + loss_gen_reenc_sum + loss_gen_redec_sum
                ...
                model_manager.log_manager.add_scalar('losses', 'loss_gen_enc', loss_gen_enc_sum + loss_gen_reenc_sum, it=it)
                model_manager.log_manager.add_scalar('losses', 'loss_gen_dec', loss_gen_dec_sum + loss_gen_redec_sum, it=it)
