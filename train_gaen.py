                ...
                with model_manager.on_step(['encoder', 'decoder', 'generator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        if g_reg_every > 0 and it % g_reg_every == 0:
                            if alt_reg:
                                reg_gen_enc = (1 / batch_mult) * g_reg_every * d_reg_param * (1. / (compute_grad_reg(labs_enc, images) + 1.))
                                model_manager.loss_backward(reg_gen_enc, nets_to_train, retain_graph=True)
                                reg_gen_enc_sum += reg_gen_enc.item()

                                reg_gen_enc = (1 / batch_mult) * g_reg_every * d_reg_param * (1. / (compute_grad_reg(labs_enc, lat_enc) + 1.))
                                model_manager.loss_backward(reg_gen_enc, nets_to_train, retain_graph=True)
                                reg_gen_enc_sum += reg_gen_enc.item()
                            else:
                        ...
                        if g_reg_every > 0 and it % g_reg_every == 0:
                            if alt_reg:
                                reg_gen_dec = (1 / batch_mult) * g_reg_every * d_reg_param * (1. / (compute_grad_reg(labs_dec, images_dec) + 1.))
                                model_manager.loss_backward(reg_gen_dec, nets_to_train, retain_graph=True)
                                reg_gen_dec_sum += reg_gen_dec.item()

                                reg_gen_dec = (1 / batch_mult) * g_reg_every * d_reg_param * (1. / (compute_grad_reg(labs_dec, lat_gen) + 1.))
                                model_manager.loss_backward(reg_gen_dec, nets_to_train, retain_graph=True)
                                reg_gen_dec_sum += reg_gen_dec.item()
                            else:
