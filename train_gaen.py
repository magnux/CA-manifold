                ...
                with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        if d_reg_every > 0 and it % d_reg_every == 0:
                            ...
                            model_manager.loss_backward(reg_dis_enc, nets_to_train, 0, retain_graph=True)
                            reg_dis_enc_sum += reg_dis_enc.item()

                            reg_dis_enc = (1 / batch_mult) * d_reg_every * d_reg_param * compute_grad_reg(labs_enc, lat_enc).mean()
                            model_manager.loss_backward(reg_dis_enc, nets_to_train, 1, retain_graph=True)
                            reg_dis_enc_sum += reg_dis_enc.item()

                        model_manager.loss_backward(loss_dis_enc, nets_to_train, 2)
                        ...
                        if d_reg_every > 0 and it % d_reg_every == 0:
                            ...
                            model_manager.loss_backward(reg_dis_dec, nets_to_train, 3, retain_graph=True)
                            reg_dis_dec_sum += reg_dis_dec.item()

                            reg_dis_dec = (1 / batch_mult) * d_reg_every * d_reg_param * compute_grad_reg(labs_dec, lat_gen).mean()
                            model_manager.loss_backward(reg_dis_dec, nets_to_train, 4, retain_graph=True)
                            reg_dis_dec_sum += reg_dis_dec.item()

                        model_manager.loss_backward(loss_dis_dec, nets_to_train, 5)
                ...
                with model_manager.on_step(['encoder', 'decoder', 'generator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        if not alt_reg and g_reg_every > 0 and it % g_reg_every == 0:
                            ...
                            model_manager.loss_backward(reg_gen_enc, nets_to_train, 6, retain_graph=True)
                            reg_gen_enc_sum += reg_gen_enc.item()

                        loss_gen_enc = (1 / batch_mult) * compute_gan_loss(labs_enc, 0)
                        model_manager.loss_backward(loss_gen_enc, nets_to_train, 7)
                        ...
                        if not alt_reg and g_reg_every > 0 and it % g_reg_every == 0:
                            ...
                            model_manager.loss_backward(reg_gen_dec, nets_to_train, 8, retain_graph=True)
                            reg_gen_dec_sum += reg_gen_dec.item()

                        loss_gen_dec = (1 / batch_mult) * compute_gan_loss(labs_dec, 1)
                        model_manager.loss_backward(loss_gen_dec, nets_to_train, 9)
                        ...
                        if alt_reg and g_reg_every > 0 and it % g_reg_every == 0:
                            ...
                            model_manager.loss_backward(reg_gen_dec, nets_to_train, 10)
                            reg_gen_dec_sum += reg_gen_dec.item()
