                ...
                with model_manager.on_step(['encoder', 'decoder', 'generator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        if g_reg_every > 0 and it % g_reg_every == 0:
                            if alt_reg:
                                reg_gen_enc = (1 / batch_mult) * g_reg_every * d_reg_param * (100. / (1. + compute_grad_reg(lat_enc, images)))
                        ...
                        if g_reg_every > 0 and it % g_reg_every == 0:
                            if alt_reg:
                                reg_gen_dec = (1 / batch_mult) * g_reg_every * d_reg_param * (100. / (1. + compute_grad_reg(images_dec, lat_gen)))
