        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                if d_reg_every_mean > 0 and it % d_reg_every_mean == 0:
                    d_reg_factor = d_reg_every_mean_next * (1 / d_reg_param_mean)
                else:
                ...
                with model_manager.on_step(['encoder', 'labs_encoder', 'decoder', 'generator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        if g_reg_every_enc > 0 and it % g_reg_every_enc == 0:
                            reg_gen_enc, pl_mean_enc = compute_pl_reg(lat_enc, images, pl_mean_enc, reg_factor=g_reg_factor_enc)
                        ...
                        if g_reg_every_dec > 0 and it % g_reg_every_dec == 0:
                            # if alt_reg:
                            #     reg_gen_dec, pl_mean_dec = compute_pl_reg(images_dec, lat_gen, pl_mean_dec, alt_pl=g_reg_factor_dec, mode='dct')
                            # else:
                            reg_gen_dec, pl_mean_dec = compute_pl_reg(images_dec, lat_gen, pl_mean_dec, reg_factor=g_reg_factor_dec)
