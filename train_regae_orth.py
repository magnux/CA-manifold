...
model_manager = ModelManager('regae_orth', networks_dict, config)  #, to_avg=['encoder', 'decoder'])
...
# # Distributions
# zdist_mu, zdist_cov = load_multigauss_params(config['training']['out_dir'] + '/regae_orth', lat_size, device=device)
# zdist_cov_inv = None
...
for _ in range(model_manager.epoch, n_epochs):
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['encoder', 'decoder'] + (['letter_encoder', 'letter_decoder'] if letter_encoding else [])) as nets_to_train:
                    ...
                    for b in range(batch_mult):
                        ...
                        with torch.no_grad():
                            ...
                            images_init = (1 - nf) * images[train_perm] + nf * noise_init
                            images_target = (1 - nf) * images + nf * noise_init
                ...
                if config['training']['stream_every'] > 0 and (model_manager.it % config['training']['stream_every']) == 0:
                    ...
                    stream_images(images_dec, config_name + '/regae_orth', config['training']['out_dir'] + '/regae_orth')
    ...
    # save_multigauss_params(zdist_mu, zdist_cov, config['training']['out_dir'] + '/regae_orth')
