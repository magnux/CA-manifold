...
model_manager = ModelManager('regae_rw', networks_dict, config)  #, to_avg=['encoder', 'decoder'])
...
# # Distributions
# zdist_mu, zdist_cov = load_multigauss_params(config['training']['out_dir'] + '/regae_rw', lat_size, device=device)
# zdist_cov_inv = None
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['encoder', 'decoder'] + (['letter_encoder', 'letter_decoder'] if letter_encoding else [])) as nets_to_train:
                    ...
                    for b in range(batch_mult):
                        ...
                        with torch.no_grad():
                            ...
                            images_target = (1 - nf_next) * images + nf_next * noise_init
                        ...
                        noise_out = decoder(None, images_init, labels, torch.ones((batch_split_size, 1), device=device))

                        loss_rw = (1 / batch_mult) * F.mse_loss(noise_out, noise_init)
                        model_manager.loss_backward(loss_rw, nets_to_train)
                        loss_rw_sum += loss_rw.item()

                model_manager.set_err(loss_dec_sum + loss_denoise_sum + loss_rw_sum)
                ...
                if config['training']['stream_every'] > 0 and (model_manager.it % config['training']['stream_every']) == 0:
                    ...
                    stream_images(images_dec, config_name + '/regae_rw', config['training']['out_dir'] + '/regae_rw')
                ...
                # Print progress
                running_loss[batch % window_size] = loss_dec_sum + loss_denoise_sum + loss_rw_sum
                ...
                model_manager.log_scalar('losses', 'loss_rw', loss_rw_sum)

    # save_multigauss_params(zdist_mu, zdist_cov, config['training']['out_dir'] + '/regae_rw')
