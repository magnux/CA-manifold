...
from src.utils.loss_utils import compute_grad_reg, compute_gan_loss, update_reg_params, update_g_factors
    ...
    with model_manager.on_epoch(epoch):
        ...
        for batch in t:
            ...
            with model_manager.on_batch():
                ...
                if not (g_reg_every > 0 and it % g_reg_every == 1):
                    ...
                    for _ in range(batch_mult):
                        ...
                        if one_dec_pass:
                            ...
                            if config['training']['through_grads']:
                                ...
                                images_redec, _, _ = decoder(lat_gen.detach().clone(), out_embs[-1].detach().clone(), seed_n=(it % (n_seed - 1)) + 1)
