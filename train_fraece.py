...
parser = argparse.ArgumentParser(description='Train a FRAE-CE')
...
model_manager = ModelManager('fraece', networks_dict, config)
    ...
    for epoch in range(model_manager.start_epoch, config['training']['n_epochs'] // 8):
        with model_manager.on_epoch(epoch):
            ...
            for batch in t:
                with model_manager.on_batch():
                    ...
                    with model_manager.on_step(['encoder', 'decoder', 'generator']) as nets_to_train:
                        for _ in range(batch_split):
                            ...
                            loss_dec = (1 / batch_split) * 1e-2 * F.cross_entropy(images_dec_raw[1], ((images + 1) * 127.5).long())
                ...
                stream_images(images_dec, config_name + '/fraece_pretrain', config['training']['out_dir'] + '/fraece_pretrain')
...
for epoch in range(model_manager.start_epoch, config['training']['n_epochs']):
            ...
            with model_manager.on_batch():
                ...
                with model_manager.on_step(['dis_encoder', 'discriminator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        with torch.no_grad():
                            ...
                            _, out_embs, images_dec_raw = decoder(lat_gen)
                            if one_dec_pass:
                        ...
                        images_redec_raw[0].requires_grad_()
                        lat_top_dec, _, _ = dis_encoder(images_redec_raw[0], lat_gen)
                        labs_dec = discriminator(lat_top_dec, labels)

                        if d_reg_every_mean > 0 and it % d_reg_every_mean == 0:
                            reg_dis_dec = (1 / batch_mult) * d_reg_factor * compute_grad_reg(labs_dec, images_redec_raw[0])
                ...
                with model_manager.on_step(['encoder', 'decoder', 'generator']) as nets_to_train:
                    ...
                    for _ in range(batch_mult):
                        ...
                        loss_dec = (1 / batch_mult) * 1e-2 * F.cross_entropy(images_dec_raw[1], ((images + 1) * 127.5).long())
                        ...
                        lat_top_dec, _, _ = dis_encoder(images_redec_raw[0], lat_gen.clone().detach())
                        labs_dec = discriminator(lat_top_dec, labels)
                ...
                with torch.no_grad():
                    ...
                    images_gen, out_embs, images_gen_raw = decoder(lat_gen)
                    if one_dec_pass:
                        images_regen_raw = images_gen_raw
                    else:
                        images_regen, _, images_regen_raw = decoder(lat_gen, out_embs[-1])
                        images_gen = torch.cat([images_gen, images_regen], dim=3)
                    images_gen = torch.cat([images_gen, images_regen_raw[0]], dim=3)

                stream_images(images_gen, config_name + '/fraece', config['training']['out_dir'] + '/fraece')
    ...
    with torch.no_grad():
        ...
        if config['training']['sample_every'] > 0 and ((epoch + 1) % config['training']['sample_every']) == 0:
            ...
            z_enc, _, _ = encoder(images, labels)
            lat_enc = generator(z_enc, labels)
            images_dec, out_embs, images_dec_raw = decoder(lat_enc)
            if one_dec_pass:
                images_redec_raw = images_dec_raw
            else:
                images_redec, _, images_redec_raw = decoder(lat_enc, out_embs[-1])
                images_dec = torch.cat([images_dec, images_redec], dim=3)
            images_dec = torch.cat([images_dec, images_redec_raw[0]], dim=3)
